...
var mouse = Vector2()
@onready var stabbert: Node3D = $stabbert/Handle


const DIST = 1000 #Ray Max distance
@onready var static_body_3d___floor: StaticBody3D = $"StaticBody3D - Floor"

func _process(delta: float) -> void:
	if grabbed_object and grabbed_object != static_body_3d___floor:
		if grabbed_object == stabbert:
			lift_item(grabbed_object,get_grab_position(),delta)
		elif grabbed_object != static_body_3d___floor:
			grabbed_object = null
		else:
